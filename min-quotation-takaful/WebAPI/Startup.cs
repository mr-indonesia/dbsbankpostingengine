...
namespace WebAPI
{
    public class Startup
    {
        ...
        public void ConfigureServices(IServiceCollection services)
        {
            ...
            {
                c.SwaggerDoc("v1", new OpenApiInfo { Title = "TakafulQuotationService", Version = "v1" });
				// Configure to handle larger responses
				c.MapType<byte[]>(() => new OpenApiSchema { Type = "string", Format = "binary" });
				//Disable Try It Out for Problematic Endpoints
			...
			GlobalConfiguration.Configuration.UseSqlServerStorage(Configuration.GetConnectionString("HangfireConnection")).WithJobExpirationTimeout(TimeSpan.FromDays(7));
			/*services.AddHangfire(config => config.SetDataCompatibilityLevel(CompatibilityLevel.Version_170)
                                            .UseSimpleAssemblyNameTypeSerializer()
                                            .UseDefaultTypeSerializer()
                                            .UseMemoryStorage());*/
            ...
            services.AddScoped<IAuthenticationHandler, AuthenticationHandlerService>();
                              ...
                              ,ISchedulerService schedulerService)
        {
            if (env.IsDevelopment() || env.IsProduction())
            {
				app.UseSwagger(c =>
				{
					c.SerializeAsV2 = false; // Use V3 for better large response handling
				...
				app.UseSwaggerUI(c =>
				{
					...
					{
						["activated"] = false // Disable syntax highlighting for large responses
					};
				});
            ...
            app.UseEndpoints(endpoints =>
            {
                ...
                endpoints.MapGet("/", async context => {
                    await context.Response.WriteAsync("Welcome to Quotation Non Health service");
                });
            });
            ...
            jobsDashboardUrl = jobsDashboardUrl == null ? "QuotationJobs" : jobsDashboardUrl;
	...
	public class SwaggerExcludeFilter : IOperationFilter
	{
		...
		{
			if (context.MethodInfo.Name == "GetLargeData")
			{
				operation.Extensions.Add("x-disable-try-it-out", new OpenApiBoolean(true));
			}
		}
	}
}
